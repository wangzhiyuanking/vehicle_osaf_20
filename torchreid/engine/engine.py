...
ax0 = fig.add_subplot(121, title="epoch-lr")
ax1 = fig.add_subplot(122, title="loss-epoch")
train_lr = []
train_loss = []
...
class Engine(object):
    ...
    def train(self, print_freq=10, fixbase_epoch=0, open_layers=None):
        ...
        for self.batch_idx, data in enumerate(self.train_loader):
            ...
            loss_summary,loss_total = self.forward_backward(data)
        ...
        train_lr.append(self.get_current_lr())
        # self.draw_curve(cur_epoch)
        self.update_lr()
    def draw_curve(self,current_epoch):
        x_epoch.append(current_epoch)
        ax0.plot(x_epoch, train_lr, 'bo-', label='train')
        ax1.plot(x_epoch, train_loss, 'bo-', label='train')
        if current_epoch == 0:
            ax0.legend()
            ax1.legend()
        # fig.savefig(os.path.join('/home/eini/WZY/OSAF','veri_drop0.2_lr0.25train.jpg'))
    ...
    def _evaluate(
        ...
        if rerank:
            print('Applying vehicle re-ranking ...')
